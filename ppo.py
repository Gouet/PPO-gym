...
def update(actorCritic, time_horizon, ppo_epochs, mini_batch_size, states, actions, log_probs, returns, advantages, grad_clip=0.5, value_factor=1, entropy_factor=0.0):
    ...
    for epoch in range(ppo_epochs):
        for i in range(int(time_horizon / mini_batch_size)):
            ...
            batch_obs = _pick_batch(mini_batch_size, states, i, shape=actorCritic.state_shape)
...
class ActorCritic():
    def __init__(self, num_action, lr, clip_param, final_step, state_shape, filename='./saved/actor.h5'):
        ...
        x = tf.keras.layers.Dense(150, activation=tf.nn.relu, kernel_initializer=w)(x)
        x = tf.keras.layers.Dense(120, activation=tf.nn.relu, kernel_initializer=w)(x)
